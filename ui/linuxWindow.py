...
from PySide.QtGui import QWidget, QHBoxLayout, QMenuBar, QAction, QIcon, QFrame, QVBoxLayout, \
    QToolBar, QLabel, QTableWidget, QTableWidgetItem, QLineEdit, QPushButton, QComboBox, QSplitter,\
    QApplication, QTextBrowser, QDialog, QTabWidget, QCompleter, QStringListModel, QRadioButton
sys.path.append('..')
...
class MainWindow(QDialog):
    ...
    def _set_device(self):
        ...
        try:
            ...
            while "" in deviceList:
                deviceList.remove("")
            # if self.androidDeviceAction.isChecked():
            #     deviceList = ['android']
            if len(deviceList):
    ...
    def get_device_status(self):
        ...
        """
        self._set_device()
        mythread = device.ThreadDeviceStatus(app_type = 'linux')
    ...
    def stop(self):
        try:
            common.Log.info("stop monkey")
            global deviceList
            self.run.stop(deviceList[0], end=True)
            self.startButton.setIcon(QIcon(static.startPNG))

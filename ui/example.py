#!/usr/bin/env python
# -*- coding:utf-8 -*-
from PySide.QtGui import QSpacerItem,QSizePolicy, QWidget,QHBoxLayout,QTableWidget,QPushButton,QApplication,QVBoxLayout,QTableWidgetItem, QCheckBox,QAbstractItemView,QHeaderView,QLabel,QFrame
# from PySide import QtWidgets
from PySide.QtCore import Qt,QTextCodec
...
windowTital = codec.toUnicode("欢迎加入微信公众号:python玩转网络 ")
add = codec.toUnicode("增加")
delete = codec.toUnicode("删除")
edit = codec.toUnicode("可以编辑")
edit = codec.toUnicode("选择整行")
edit = codec.toUnicode("禁止选多行")
edit = codec.toUnicode("以分数排序")
edit = codec.toUnicode("标头设置")
edit = codec.toUnicode("文字居中加颜色")
edit = codec.toUnicode("取消边框颜色交替")
edit = codec.toUnicode("选择")
edit = codec.toUnicode("姓名")
edit = codec.toUnicode("成绩")
edit = codec.toUnicode("住址")
edit = codec.toUnicode("自动生成随机一行数据！,checkbox设置为居中显示")
edit = codec.toUnicode("可以编辑")
edit = codec.toUnicode("可以编辑")
...
#     # Sess = sessionmaker(bind = engine)
    ...
    def setheader(self):
        ...
        self.settext(u'设置标头字体及字号，隐藏ID列，设置标头除姓名外全部为固定宽度\n，设置姓名列自动扩展宽度，设置标头行高，设置标头背景色')
    def middle(self):
